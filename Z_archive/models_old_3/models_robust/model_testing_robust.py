...
Layout
  ...
  G4  realised_breakdown internal consistency + in-box clip==no-clip
...
def random_inputs(fp, price_model):
    ...
    h_plus = np.abs(RNG.normal(0.5, 0.2, T)) + 0.2
    h_minus = np.abs(RNG.normal(0.5, 0.2, T)) + 0.2
    pi_da = RNG.uniform(20, 60, T)
    vals = {"pl_hat": pl_hat, "h_plus": h_plus, "h_minus": h_minus, "pi_da": pi_da}
...
def _solve_vertex_reference(fp, price_model, vals, solver):
    ...
    pl_hat, hp, hm = vals["pl_hat"], vals["h_plus"], vals["h_minus"]
    ...
    # explicit robust feasibility at EVERY box vertex (numeric vertices)
    for signs in itertools.product([1, -1], repeat=T):
        xiv = np.where(np.array(signs) > 0, hp, -hm)
        cons += [pch + Dch @ xiv >= 0, pch + Dch @ xiv <= fp.C_ch,
                 pdis + Ddis @ xiv >= 0, pdis + Ddis @ xiv <= fp.C_dis,
                 s_hat + G @ xiv >= 0, s_hat + G @ xiv <= fp.B_max]
...
def gate_vertex(solver_name="GUROBI"):
    ...
    for pm, k in CORNERS:
        ...
        # feasibility of the LAYER solution at every vertex
        feas = _vertices_feasible(fp, out, vals)
        _P(f"layer solution feasible at all {2**fp.T_total} vertices  [{pm}, k={int(k)}]", feas)


def _vertices_feasible(fp, dec, vals, tol=1e-5):
    T, dt = fp.T_total, fp.dt
    pch, pdis = np.asarray(dec["p_ch_hat"]), np.asarray(dec["p_dis_hat"])
    Dch, Ddis = np.asarray(dec["D_ch"]), np.asarray(dec["D_dis"])
    hp, hm = vals["h_plus"], vals["h_minus"]
    L = np.tril(np.ones((T, T)))
    s_hat = fp.SOC0 + dt * (L @ (fp.eta_ch * pch - (1.0 / fp.eta_dis) * pdis))
    Gm = dt * (L @ (fp.eta_ch * Dch - (1.0 / fp.eta_dis) * Ddis))
    for signs in itertools.product([1, -1], repeat=T):
        xiv = np.where(np.array(signs) > 0, hp, -hm)
        if (pch + Dch @ xiv < -tol).any() or (pch + Dch @ xiv > fp.C_ch + tol).any(): return False
        if (pdis + Ddis @ xiv < -tol).any() or (pdis + Ddis @ xiv > fp.C_dis + tol).any(): return False
        if (s_hat + Gm @ xiv < -tol).any() or (s_hat + Gm @ xiv > fp.B_max + tol).any(): return False
    return True
