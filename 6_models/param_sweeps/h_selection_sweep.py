...
Ported from models_robust/h_sweep_old/h_selection_sweep.py to the modular dispatch
pipeline (dispatch_setup.setup_full_robust / dispatch_shared.py / dispatch_wrapper.py)
instead of models_robust/dispatch_layer_robust.py / dispatch_wrapper_robust.py.
Methodology, metrics, and CLI are otherwise UNCHANGED -- see this docstring's original
for the full rationale, reproduced below.
...
get_prices() now returns "pi_imb_up"/"pi_imb_down" keys (dispatch_wrapper.py's
convention), matching the modular build_objective_2stage's Parameter names exactly --
the old dispatch_wrapper_robust.get_prices returned "imb_up"/"imb_down" instead, which
would KeyError against this problem's param_by_name.
"""
...
def boxes_from_quantiles(quantiles, mean, quantile_levels, box_levels=BOX_LEVELS, min_box=MIN_BOX):
    ...
    Returns {box_level: (h_plus, h_minus)} as numpy. Matches dispatch_wrapper_robust.compute_box
...
def run_sweep(windows, forecast_fn, sampler, quantile_levels):
    ...
    # NOTE: gamma left at its default (1e-4, unified across all formulations -- was 1e-6
    # here specifically before dispatch_setup.py consolidated the three FixedParams
    # constructors). Pass gamma= explicitly if this sweep needs to match a specific
    # deployed configuration instead of the current default.
    fps = {k: default_fixed_params(num_scenarios=N_SCEN) for k in (0.0, 1.0)}
    bundles = {(pm, k): build_problem(fps[k], mode_from_corner(pm, k)) for pm, k in CORNERS}
    ...
    for d in range(n_days):
        ...
        Sigma = cholesky_of_second_moment(xi)                              # (T,T), for k=1 (torch, matches dispatch_wrapper's signature)
        ...
        for pm, k in CORNERS:
            ...
            fp = fps[k]
            bundle = bundles[(pm, k)]
            prices = get_prices(price_day, pm, cols=PRICE_COLS)             # {pi_da, pi_imb} | {pi_da,pi_imb_up,pi_imb_down}
